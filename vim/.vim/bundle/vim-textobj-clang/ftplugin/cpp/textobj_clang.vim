...
if g:textobj_clang_more_mapping
    ...
    for s:name in g:textobj_clang_mapping_kinds
        let s:spec[s:name] = {
                    \ 'select-i' : 'ix'.s:name[0],
                    \ '*select-i-function*' : 'textobj#clang#' . s:name . '_select_i',
                    \ 'select-a' : 'ix'.s:name[0],
                    \ '*select-a-function*' : 'textobj#clang#' . s:name . '_select_i',
                    \ }
    endfor
else
    let s:spec['-'] = {
                \ 'select-i' : 'ix',
                \ '*select-i-function*' : 'textobj#clang#under_cursor_select_i',
                \ 'select-a' : 'ax',
                \ '*select-a-function*' : 'textobj#clang#any_select_i',
                \ }
endif
